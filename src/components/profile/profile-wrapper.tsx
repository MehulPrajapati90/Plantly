...
const ProfilePreviewWrapper = ({ workspace, workspacedata }: PreviewProps) => {
    ...
    // const name = `${workspacedata?.user?.firstName} ${workspacedata?.user?.lastName}`;
        ...
        <div className="min-h-auto p-6 flex">
            ...
            <div className="w-[400px] h-150 bg-black rounded-[30px] border border-[#3b3b3f] py-10 relative overflow-y-auto hidden-scrollbar flex flex-col justify-between items-center">
                <div className="w-full h-auto">
                    ...
                    <div className="w-full flex flex-col items-center pt-8">
                        <div className="size-18 rounded-full flex items-center justify-center bg-zinc-700 border border-zinc-600 relative">
                            <Image height={20} width={20} unoptimized src={`${workspacedata?.imageUrl}`} alt="profile image" className="size-16 rounded-full" />
                        ...
                        <div className="pt-10 flex gap-3">
                            {workspacedata?.socialLinks?.map(({ platform, url }: { platform: string; url: string }, idx: number) => {
                ...
                <div className="w-full px-5 pt-8 pb-6 flex flex-col justify-center gap-3">
                    {workspacedata?.link.map(({ description, profileImageUrl, title, url }: { description: string | null; profileImageUrl: string | null; title: string; url: string }, idx: number) => (
                        <Link href={url} key={idx} target="_blank" className="w-full flex justify-center items-center relative bg-white/10 border-white/10 border hover:bg-white/20 transition-all duration-200 ease-in-out h-11 rounded-[8px]">
                            <Image src={`${profileImageUrl}`} unoptimized height={20} width={20} alt="link profile" className="size-7 rounded-full absolute left-3" />
                            <span className="text-white font-sans font-medium tracking-[-0.4px] text-[15px]">{title}</span>
                        </Link>
                    ))}
                </div>
                ...
                <div className="flex w-full flex-col items-center justify-center">
                    ...
                    <div className="w-full min-h-10 flex justify-between items-center px-5">
                        <Link href={`${process.env.NEXT_PUBLIC_FRONTEND_URL}/${redirectPath}`} target="_blank" className="w-full min-h-10 flex justify-between items-center px-10 bg-white/10 backdrop-opacity-30 backdrop-blur-[10px] rounded-[8px] cursor-pointer hover:bg-white/15 mt-6 transition-all ease-in-out duration-200">
                            <Rocket size={18} />
                            <p className="text-[13px] font-sans font-medium">{`Join ${formateWorkspaceName} on Plantly`}</p>
