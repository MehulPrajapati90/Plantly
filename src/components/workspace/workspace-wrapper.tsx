...
import UpdateUserWorkspaceModal from "./workspace-profile-update-modal";

interface WorkspaceWrapperProps {
    username: string
}

const WorkspaceWrapper = ({ username }: WorkspaceWrapperProps) => {
    ...
    return (
        <div className="w-full min-h-auto flex flex-col justify-center items-center">
                ...
                <div className="flex justify-center items-center gap-5 pt-5">
                    <div className="w-full md:w-[50%] flex flex-col gap-10">
                        <div className="w-full min-h-auto bg-zinc-900 rounded-[100px] border border-dashed border-zinc-500">
                            <WorkspaceProfile workspace={username} workspacedata={data?.workspacedata!} />
                        </div>
                    ...
                    <div className="w-full md:w-[50%] max-h-140 bg-zinc-900 rounded-[30px] border border-dashed border-zinc-500 hidden md:flex min-h-130">
                        <Preview workspace={username} workspacedata={data?.workspacedata!} />
                    </div>
                </div>
            </div>
            <WorkSpaceModal />
            ...
            <UpdateUserWorkspaceModal workspace={username} workspacedata={data?.workspacedata!} />
        </div>
    )
}
